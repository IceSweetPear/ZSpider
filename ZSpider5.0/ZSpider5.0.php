        ...
        foreach ($configs as $configName => $config) {
            ...
            while ($index < 2) {
                ...
                if ($pid == -1) {
                    ...
                    echo 'i am parent'.$pid."\n";

                    pcntl_wait($status, WNOHANG);

                } elseif ($pid == 0) {

                    echo 'i am child'.posix_getpid()."\n";

                    $sleepCount = 3;

                    while (true) {
                        $dataQueue = $this->redis->get('data_queue');
                        if (is_string($dataQueue)){
                            exit();
                        }
                        if (empty($dataQueue)){
                        ...
                        $dataInfo = array_shift($dataQueue);
                        $this->redis->put('data_queue', $dataQueue);
