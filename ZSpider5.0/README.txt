...
6.1
	...
	get set 是字符串 pop 是数组 怎么结合在一起呢？

	规定 一个 文件对应一个key 一个文件里存放 任意东西  字符串 数组 对象 等 都可以放

	但是呢 有 约束   只存字符串的时候 其实相当于   key =>   字符串     key  是文件名  字符串 是内容   在 文件名 于  文件内容 构建了一个数组
	
	那么  我完全可以 内容是 数组 或者 字符串 那现在 我就不能 pop了 因为我取数组 的一个 字符串的值 再pop 也不行啊 所以没有 硬性的要求 跟数组一样就

	我的爬虫要怎么升级呢？

	缓存文件方式?感觉不好使 频繁的从文件中读取 能行吗? 百度百度吧
	
	一个key,一个文件 也可以使用md5

	那如何 在指定key下追加呢?  这是一个问题

	我有了 读取文件  读取到数组里 完成追加 直接写入 我看还行  试试把
	
6.2
	本次有几个想法 
	
	1，保存爬取进度  我的这个很好实现 增加一个方法就行  给对象增加析构函数 然后 在停止的时候 
	
		截取当前存储区的进度 并保存 保存当前的所有任务 至 文件缓存 键为一个随机的10位字符 
	
		你复制上这个10位的字符 至参数  下次 脚本 识别 key 获取 任务 填充任务 开始 

		我们要做的 一个是结束截取 一个 开始 填充 没了
